...
function PatientProfile() {
  ...
  const fetchPatientProfile = useCallback(async (token) => {
    try {
      setLoading(true);
      
      console.log("Fetching patient profile with token:", token.substring(0, 20) + "...");
      ...
      if (err.response?.status === 401) {
        ...
        const errorMsg = err.response.data?.error || "Bad request. Please check your account setup.";
        setError(errorMsg);
      } else {
    ...
    return (
      <Box sx={{ maxWidth: 1200, mx: "auto", display: "block" }}>
        ...
        <Card sx={{ mb: 2, p: 2 }}>
          ...
          <Typography>Response Data: {JSON.stringify(debugInfo.data, null, 2)}</Typography>
        </Card>
    ...
    <Box sx={{ maxWidth: 1200, mx: "auto", display: "block" }}>
      ...
      <Grid >
        ...
        <Grid item xs={12}>
          ...
          >
            <Box >
              ...
              <InfoRow label="Age" value={patient.age ? `${patient.age} years` : "N/A"} />
              <InfoRow label="Marital Status" value={patient.maritalStatus || "N/A"} />
              <InfoRow label="Occupation" value={patient.occupation || "N/A"} />
              <InfoRow label="Education" value={patient.educationStatus || "N/A"} />
            </Box>
          </InfoSection>
        </Grid>
        ...
        <Grid item xs={12}>
          ...
          >
            <Box >
              <InfoRow label="Assigned Caregiver" value={patient.caregiverName || "N/A"} />
              <InfoRow label="Caregiver Phone" value={patient.caregiverPhone || "N/A"} />
              <InfoRow label="Care Plan" value={patient.carePlan || "Standard Recovery Plan"} />
              <InfoRow label="Medical History" value={patient.medicalHistory || "None"} />
              <InfoRow label="Physical Issues" value={patient.physicalHealthProblems || "None"} />
              <InfoRow label="Substance Use" value={patient.substanceAbuseCharacteristics || "None"} />
            </Box>
          </InfoSection>
        </Grid>
        ...
        <Grid item xs={12}>
          ...
          >
            <Box >
              ...
              <InfoRow label="Allergies" value={patient.allergies || "No known allergies"} />
              ...
              <InfoRow 
                label="Last Updated" 
                value={patient.updated_at ? formatDate(patient.updated_at) : "N/A"} 
              />
            </Box>
          </InfoSection>
        </Grid>
